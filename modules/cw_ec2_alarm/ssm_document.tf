  ...
  - name: listblockdevice
    ...
    inputs:
      ...
      Parameters:
        commands:
          - '#!/bin/sh '
          - 'lsblk -l --output KNAME,MOUNTPOINT,FSTYPE,NAME  | grep "/" | grep -v "loop" | awk ''BEGIN {printf"{\"discarray\":["}{if($1=="KNAME")next;if(a)printf",";printf"{\"Filesystem\":\""$1"\",\"mount\":\""$2"\",\"fstype\":\""$3"\",\"name\":\""$4"\"}";a++;}END{print"]}";}'''
    outputs:
      - Name: blockdevice
        Selector: $.Output
        Type: String
